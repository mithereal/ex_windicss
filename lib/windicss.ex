defmodule Windicss do
  # https://github.com/windicss/windicss/releases
  @latest_version "3.5.6"

  @moduledoc """
  Windicss is an installer and runner for [windicss](https://windicss.org/).

  ## Profiles

  You can define multiple windicss profiles. By default, there is a
  ...
  There are two global configurations for the windicss application:

    * `:version` - the expected windicss version

    * `:path` - the path to find the windicss executable at. By
      default, it is automatically downloaded and placed inside
      the `_build` directory of your current app

  Overriding the `:path` is not recommended, as we will automatically
  download and manage `windicss` for you. But in case you can't download
  it (for example, GitHub behind a proxy), you may want to
  set the `:path` to a configurable system location.

  For instance, you can install `windicss` globally with `npm`:

      $ npm install -g windicss

  On Unix, the executable will be at:

      NPM_ROOT/windicss/node_modules/windicss-TARGET/bin/windicss

  On Windows, it will be at:

      NPM_ROOT/windicss/node_modules/windicss-windows-(32|64)/windicss.exe
  ...
  `MIX_Windicss_PATH` environment variable, which you can then read in
  your configuration file:

      config :windicss, path: System.get_env("")
  ...
  *Note*: The stand-alone Windicss client bundles first-class windicss packages
  within the precompiled executable. For third-party Windicss plugin support,
  the node package must be used. See the
  [windicss nodejs installation instructions](https://windicss.org/docs/installation)
  ...
  @doc false
  def start(_, _) do
    unless Application.get_env(:windicss, :version) do
      Logger.warn("""
      windicss version is not configured. Please set it in your config files:

          config :windicss, :version, "#{latest_version()}"
      """)
    end
    ...
    case bin_version() do
      ...
      {:ok, version} ->
        Logger.warn("""
        Outdated windicss version. Expected #{configured_version}, got #{version}. \
        Please run `mix windicss.install` or update the version in your config files.\
        """)
  ...
  @doc """
  Returns the configured windicss version.
  """
  def configured_version do
    Application.get_env(:windicss, :version, latest_version())
  end
  ...
  def config_for!(profile) when is_atom(profile) do
    Application.get_env(:windicss, profile) ||
      raise ArgumentError, """
      unknown windicss profile. Make sure the profile is defined in your config/config.exs file, such as:
  ...
  """
  def bin_path do
    name = "windicss-#{target()}"

    Application.get_env(:windicss, :path) ||
  ...
  @doc """
  Returns the version of the windicss executable.
  ...
  def bin_version do
    ...
    with true <- File.exists?(path),
         ...
         [vsn] <- Regex.run(~r/windicss v([^\s]+)/, out, capture: :all_but_first) do
      {:ok, vsn}
    else
      _ -> :error
    end
  ...
  @doc """
  Installs, if not available, and then runs `windicss`.
  ...
  @doc """
  The default URL to install Windicss from.
  """
  ...
  @doc """
  Installs windicss with `configured_version/0`.
  """
  def install(base_url \\ default_base_url()) do
    url = get_url(base_url)
    bin_path = bin_path()
    windicss_config_path = Path.expand("assets/windicss.config.js")
    ...
    unless File.exists?(windicss_config_path) do
      File.write!(windicss_config_path, """
      // See the Windicss configuration guide for advanced usage
      // https://windicss.org/docs/configuration

      let plugin = require('windicss/plugin')
  ...
  # Available targets:
  #  windicss-freebsd-arm64
  #  windicss-freebsd-x64
  #  windicss-linux-arm64
  #  windicss-linux-x64
  #  windicss-linux-armv7
  #  windicss-macos-arm64
  #  windicss-macos-x64
  #  windicss-windows-x64.exe
  defp target do
    ...
    case {:os.type(), arch, :erlang.system_info(:wordsize) * 8} do
      ...
      {_os, _arch, _wordsize} -> raise "windicss is not available for architecture: #{arch_str}"
    end
  end

  defp fetch_body!(url) do
    url = String.to_charlist(url)
    Logger.debug("Downloading windicss from #{url}")
  ...
  defp cacertfile() do
    Application.get_env(:windicss, :cacerts_path) || CAStore.file_path()
  end
